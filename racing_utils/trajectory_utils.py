...
def RedGateSpawnerCircle(client, num_gates, radius, radius_noise, height_range, track_offset=[0, 0, 0]):
    track = generate_gate_poses(num_gates=num_gates, race_course_radius=radius, radius_noise=radius_noise,
                                height_range=height_range, direction=0, offset=track_offset)
    for idx in range(num_gates):
...
def RedGateSpawnerTrack(client, num_gates, radius, radius_noise, height_range, num_ignore=0, track_offset=[0, 0, 0]):
    ...
    track_0 = generate_gate_poses(num_gates=num_gates, race_course_radius=radius, radius_noise=radius_noise,
                                  height_range=height_range, direction=0, offset=offset_0)
...
def generate_gate_poses(num_gates, race_course_radius, radius_noise, height_range, direction, offset=[0, 0, 0],
                        type_of_segment="circle"):
    if type_of_segment == "circle":
        (x_t, y_t, z_t) = tuple(
            [generate_circle(i, num_gates, race_course_radius, radius_noise, direction) for i in range(3)])
        ...
        gate_poses = [airsimdroneracingvae.Pose(airsimdroneracingvae.Vector3r((x_t[t_i][0] + offset[0]),
                                                                              (y_t[t_i][0] + offset[1]),
                                                                              random.uniform(height_range[0],
                                                                                             height_range[1]) + offset[
                                                                                  2]),
...
def quaternionFromUnitGradient(dx_dt, dy_dt, dz_dt):
    ...
    q = airsimdroneracingvae.Quaternionr(
        ...
        math.sqrt((r0.x ** 2 + r0.y ** 2 + r0.z ** 2) * (dx_dt ** 2 + dy_dt ** 2 + dz_dt ** 2)) + (
                    r0.x * dx_dt + r0.y * dy_dt + r0.z * dz_dt)
    )
    # Normalize
    length = q.get_length()
    if (length == 0.0):
...
def generate_circle(i, num_gates, race_course_radius, radius_amp, direction):
    ...
    if i == 0:
        for (idx, t) in enumerate(ts):
            ...
            if idx > 0:
                radius = np.clip(radius, radius_list[idx - 1] - max_radius_delta,
                                 radius_list[idx - 1] + max_radius_delta)
