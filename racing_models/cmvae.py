...
from racing_models.dronet import Dronet
from racing_models.decoders import ImgDecoder, GateDecoder
from racing_models.transformer import NonLinearTransformer
...
class Cmvae(Model):
    def __init__(self, n_z, gate_dim=4, res=96, trainable_model=True):
        ...
        self.q_img = Dronet(num_outputs=n_z * 2, include_top=True)
        self.p_img = ImgDecoder()
        self.p_gate = GateDecoder(gate_dim=gate_dim)
        # Create sampler
...
class CmvaeDirect(Model):
    def __init__(self, n_z, gate_dim=4, res=96, trainable_model=True):
        ...
        self.q_img = Dronet(num_outputs=n_z * 2, include_top=True)
        self.p_img = ImgDecoder()
        self.p_R = NonLinearTransformer()
        self.p_Theta = NonLinearTransformer()
        self.p_Psi = NonLinearTransformer()
        self.p_Phi = NonLinearTransformer()
        # Create sampler
        self.mean_params = Lambda(lambda x: x[:, : n_z])
        self.stddev_params = Lambda(lambda x: x[:, n_z:])
